...
def load_config_v3(config_dir: Path | str | None = None) -> AppConfigV3:
    cfg = AppConfigV3()
    ...
    raw = _load_yaml(config_dir / "promptwise_v3.yaml")
    if not raw:
        raw = _load_yaml(config_dir / "promptwise_v2.yaml")
    if not raw:
    ...
    cfg.version = str(raw.get("version", "3.0"))
