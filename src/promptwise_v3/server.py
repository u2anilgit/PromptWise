"""PromptWise V3 — Unified MCP server with ALL tools."""
...
from promptwise_v3.db import init_db, SessionManager, MemoryManager
...
async def list_tools_v3() -> list[Tool]:
    return _TOOL_DEFS


async def call_tool_v3(ctx: ServerContext, name: str, arguments: dict) -> str:
    try:
        # ── Core Routing & Optimization ──────────────────────────────────────
        if name == "route_request":
            ...
            await ctx.memory.record_cost(tool="route_request", session_id="v3", model=r.recommended_model, cost_usd=r.estimated_input_cost_usd)
            ...
            await ctx.memory.record_cost(tool="rewrite_prompt", session_id="v3", model=arguments.get("model", "claude-sonnet-4-6"), input_tokens=r.raw_tokens, saving_pct=r.saving_pct)
...
async def main() -> None:
    # repo root = src/promptwise_v3/server.py -> parents[2]; config/ and skills/ live there.
    config_dir = Path(__file__).resolve().parents[2]
    config = load_config_v3(config_dir)
    ...
    async def _list_tools() -> list[Tool]:
        return await list_tools_v3()

    @server.call_tool()
    async def _call_tool(name: str, arguments: dict):
        result = await call_tool_v3(ctx, name, arguments)
        return [TextContent(type="text", text=result)]

    init_opts = InitializationOptions(
        server_name="promptwise-v3",
