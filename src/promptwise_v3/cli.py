"""PromptWise V3 — CLI entry point (stats, eval, serve)."""
...
from promptwise_v3 import __version__
from promptwise_v3.config import load_config_v3
from promptwise_v3.db.models import MemoryManager, get_db_path


def _do_stats(config_dir: str | None) -> None:
    cfg = load_config_v3(config_dir)
    ...
    print(f"PromptWise V3 — {__version__}")
...
def _start_serve(config_dir: str | None, port: int | None, cli_only: bool) -> None:
    ...
    if cli_only or not cfg.dashboard.web_enabled:
        from promptwise_v3.dashboard.cli import CLIDashboard
        dash = CLIDashboard(cfg)
        dash.render_all()
        return

    port = port or cfg.dashboard.web_port
    print(f"Starting PromptWise V3 dashboard on http://0.0.0.0:{port}")

    from promptwise_v3.dashboard.web import create_web_app
...
def main() -> None:
    parser = argparse.ArgumentParser(
        prog="promptwise-v3",
        description=f"PromptWise V3 v{__version__} — Token-aware prompt routing",
    )
