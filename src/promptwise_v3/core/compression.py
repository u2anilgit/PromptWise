...
from promptwise_v3.types import CompressionResult
...
_PLEASANTRIES = re.compile(
    ...
    r'great question[!,]?\s*|absolutely[!,]?\s*|no problem[!,]?\s*)\s*', re.I
)
_HEDGING = re.compile(
    r'\b(might|perhaps|possibly|potentially|arguably|it seems|it appears|'
    r'I think|I believe|I guess|probably|likely|maybe)\b\s*', re.I
)
...
class CompressionEngine:
    def compress(self, text: str) -> CompressionResult:
        if not text:
            return CompressionResult(original="", compressed="", tokens_saved=0, saving_pct=0.0, rules_applied=[])
        ...
        return CompressionResult(original=text, compressed=result, tokens_saved=tokens_saved, saving_pct=saving_pct, rules_applied=rules_applied)
