...
def parse_project_lockfiles(project_dir: Path) -> list[dict]:
    """Parse manifest + lockfiles under ``project_dir`` into a flat component
    list: ``[{"name", "version", "ecosystem", "resolution"}, ...]``, deduped
    by (ecosystem, name, version) with the first (direct-manifest) resolution
    winning -- same precedence ``SBOMGenerator.generate()`` has always used.

    Shared by ``SBOMGenerator`` and ``security.dependency_guard.DependencyGuard``
    so there is exactly one place in the codebase that knows how to read a
    lockfile; extracted from ``SBOMGenerator.generate()`` verbatim (WP0).
    """
    ...
    def _add(name: str, version: str, ecosystem: str, resolution: str) -> None:
        ...
        seen.add(key)
        components.append({"name": name, "version": version, "ecosystem": ecosystem, "resolution": resolution})
...
class SBOMGenerator:
    ...
    def generate(self, project_dir: Path, *, include_ai_models: bool = True,
        ...
        project_path = Path(project_dir)
        components: list[dict] = []
        for c in parse_project_lockfiles(project_path):
            purl = f"pkg:{c['ecosystem']}/{c['name']}@{c['version']}"
            components.append({
                "type": "library", "name": c["name"], "version": c["version"], "purl": purl,
                "properties": [{"name": "resolution", "value": c["resolution"]}],
            })
