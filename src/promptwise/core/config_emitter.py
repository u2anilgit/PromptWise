...
class GovernanceBundle:
    ...
    path_rules: dict[str, list[str]] = field(default_factory=dict)  # glob -> path-scoped rules
    ...
    def from_context(cls, args: dict) -> "GovernanceBundle":
        ...
        b = cls(
            ...
            path_rules=dict(args.get("path_rules", {})),
        )
        text = args.get("text")
        if text:
...
# Detector keys without their own emitter: Codex reads the shared AGENTS.md;
# Grok Build/Grok CLI natively auto-reads CLAUDE.md and the AGENTS.md family
# with zero config (confirmed against xAI's docs, 2026-08), so it rides the
# existing "claude" file rather than getting a redundant one.
_TARGET_ALIASES = {"codex": "agents", "grok": "claude"}
...
def _glob_slug(glob: str) -> str:
    """Turn a glob into a safe instruction-file slug, e.g. 'src/**/*.ts' -> 'src-ts'."""
    slug = re.sub(r"[^a-zA-Z0-9]+", "-", glob).strip("-").lower()
...
class ConfigEmitter:
    ...
    def emit_copilot_scoped(self, b: GovernanceBundle) -> dict[str, str]:
        """Path-scoped Copilot rules: one .github/instructions/<slug>.instructions.md
        per glob in ``bundle.path_rules``, each with an ``applyTo`` frontmatter.

        Empty ``path_rules`` -> {} (no extra files), so behavior is unchanged
    ...
    def _target_pairs(self, bundle: GovernanceBundle, target: str) -> list[tuple[str, str]]:
        """Return [(relative_path, rendered_content)] for a target — usually one
        file, but Copilot also fans out into path-scoped instruction files.
        """
        target = _norm_target(target)
        pairs = [(TARGETS[target], self.render(bundle, target))]
        if target == "copilot":
            pairs += list(self.emit_copilot_scoped(bundle).items())
        return pairs

    def emit_cline(self, b: GovernanceBundle) -> str:
        return f"# .clinerules — {b.project} (generated by PromptWise)\n\n{b._body()}\n"

    def emit_windsurf(self, b: GovernanceBundle) -> str:
        # Windsurf/Cascade rules are a single flat root file, same shape as
        # .clinerules — no frontmatter, no profile lookup.
        return f"# .windsurfrules — {b.project} (generated by PromptWise)\n\n{b._body()}\n"

    def emit_jetbrains(self, b: GovernanceBundle) -> str:
        # JetBrains AI Assistant rule "type" (always/manually/by-file-pattern/
        # by-model-decision) is configured in IDE settings, not in-file
        # frontmatter, so this is a flat Markdown body like emit_windsurf.
        return f"# JetBrains AI Assistant rules — {b.project} (generated by PromptWise)\n\n{b._body()}\n"

    def emit_aider(self, b: GovernanceBundle) -> str:
        # CONVENTIONS.md is the community-standard filename (Aider-AI/conventions),
        # but unlike CLAUDE.md/AGENTS.md, Aider does NOT auto-read it without the
        # user adding `read: CONVENTIONS.md` to .aider.conf.yml -- flagged in-file
        # since we don't touch that separate YAML config (own merge strategy, not
        # worth it for a single line).
        return (f"# CONVENTIONS.md — {b.project} (generated by PromptWise)\n\n"
                f"<!-- Add `read: CONVENTIONS.md` to .aider.conf.yml, or pass "
                f"`--read CONVENTIONS.md`, so aider loads this every session. -->\n\n"
                f"{b._body()}\n")

    def emit_goose(self, b: GovernanceBundle) -> str:
        # .goosehints is a flat root file, auto-loaded from the repo root up --
        # same shape as .clinerules/.windsurfrules.
        return f"# .goosehints — {b.project} (generated by PromptWise)\n\n{b._body()}\n"

    def emit_openhands(self, b: GovernanceBundle) -> str:
        # .openhands/microagents/repo.md is auto-loaded as a repository
        # microagent; frontmatter is optional for repo agents (defaults apply).
        return f"# OpenHands repo microagent — {b.project} (generated by PromptWise)\n\n{b._body()}\n"
    ...
    def sync(
        ...
        *,
        ...
        result: dict[str, str] = {}
        for t in chosen:
            for rel, rendered in self._target_pairs(bundle, t):
                dest = root / rel
