...
async def _handle_rewrite_prompt(ctx: ServerContext, arguments: dict) -> str:
    ...
    await ctx.memory.record_cost(tool="rewrite_prompt", session_id=get_current_session_id(), model=arguments.get("model", "claude-sonnet-4-6"), input_tokens=r.raw_tokens, saving_pct=r.saving_pct)
