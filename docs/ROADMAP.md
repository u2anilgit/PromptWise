...
| 14 | Injection-detection corpus refresh workflow (offline, human-reviewed — not a live ML classifier) | Not started — `injection_benchmark.py` benchmarks against a fixed corpus but has no refresh workflow. **Design brainstormed 2026-07-25** (not yet implemented): `security/corpus_store.py` (sqlite `corpus_history` append-only table, same pattern as `RiskRegister`) + external `corpus/injection_corpus.json` (additive default load into `benchmark_injection_detector`, no signature break) + two new MCP tools `review_corpus_candidates(path)` (dry-run score against current detector) and `promote_corpus_candidates(path, approve_ids?, reject_ids?, reviewer)` (merges approved cases, writes a history row with before/after precision/recall/F1). Sample source scoped to manual JSON drop-in only — auto-harvesting real false negatives was considered and deferred (needs an undesigned miss-flagging mechanism first; would push this from M to L effort). Task breakdown: corpus_store (S ~1.5h), external corpus file + wiring (S ~1h), review tool (S ~1.5h), promote tool + history write (M ~2.5h), tests (S ~1.5h), roadmap update (trivial) — total ~8h, matches original estimate. | **Still open, ~8h holds up, design ready — resume via `/writing-plans` directly (brainstorming already done).** |
