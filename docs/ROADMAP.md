...
| Deepen `sync_agent_config`/`check_portability`/etc. across Cursor, Copilot, Windsurf | The real "one plugin, every coding agent" seed. Checked 2026-08-01: Cursor/Copilot/Windsurf/JetBrains/Cline emitters already existed pre-2026-08; this row's real remaining scope is `agent_detector.py`'s `detect_agents()`, which only fingerprints claude/codex/cursor/copilot/gemini — windsurf/jetbrains/cline/aider/goose/openhands/grok have emitters but no detection probes, so `propose_agent_config`'s target auto-detection silently misses them. #16 (Aider/Goose/OpenHands/Grok CLI emitters) done separately, see gap-closure table below. | M (detector probes only, not L — emitters already exist) | Not started |
